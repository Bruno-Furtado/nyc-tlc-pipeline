...
- **Step 3 — silver** (PR #15, reworked in Step 4): `04_silver` conforms yellow+green into
...
- **Step 4 — CDF incremental rework + gold + analysis** (in progress, sliced into PRs):
  ...
  - **Gold + analysis** (next): `06_gold.*` builds `gold.obt_trips` (consumption columns + derived
    `year`/`month`/`pickup_hour`, Liquid Clustered by `(year, month)`, **no scope filter**);
    `07_verify.py` reconciles silver↔gold; `analysis/answers.sql` holds the 2 queries with the
    Jan–May 2023 scope applied here.
...
2. **Delta-native incremental (CDF).** Bronze has `delta.enableChangeDataFeed = true` from creation
   (version 0). Silver and gold read only the *new commits* of their source via `readChangeFeed`
   (`startingVersion = watermark + 1`), never the whole table. The first run has `watermark = -1`, so
   it reads the feed from version 0: a single code path, no separate full-read bootstrap.
...
5. **Validation (fail-fast).** Silver reconciles row counts per `taxi_type` against bronze
   ...
   bronze) and the low-cardinality `taxi_type` column, not a full scan. Gold will reconcile against
   silver the same way.

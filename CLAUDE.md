...
- **`source_file` lives in bronze only.** It's the ingestion idempotency key (`02_bronze.py` appends only files not already in bronze; dedup by name, so reruns are no-ops) and the source of `year`/`month` (parsed from the file name, deterministic). Silver/gold derive `year`/`month` from it during conformation but **don't persist it** — they carry `year`/`month` + `_source_version` instead. Fine-grained row→file lineage is bronze-only by design.
